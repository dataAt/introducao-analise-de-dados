...
Para entendermos, vamos começar a com a utilização desta estrutura de dados
...
Após a conversão o uso é basicamente o mesmo do dicionário, porém com os benefícios já citados. Além do que já foi dito sobre as `Series`, é importante lembrar que, estas estruturas de dados possuem diversos métodos para facilitar a manipulação e entendimento dos dados, apresentados nas próximas seções.

Mesmo as `Series` sendo estruturas de dados poderosas, elas apresentam a limitação da unidimensionalidade, o que impede estruturas como matrizes e tabelas de serem representados nesta estrutura. Para esta limitação ficar clara, vamos inserir uma matriz de dados dentro de uma `Series`, erros não serão apresentados, porém a representação pode não ficar como desejado.
...
Percebeu ? Não há uma matriz, e sim várias listas dentro de listas em cada linha. A ideia é que a matriz fosse apresentada em um formato onde cada elemento está em uma posição, assim como apresentado abaixo:
...
Boa parte dos métodos disponíveis em uma `Series` também são aplicáveis em `DataFrames`, o que ajuda no aprendizado de utilização da API do pandas, mas é importante entender que, em diversos casos o simples fato de haver mais dimensões nos dados torna a manipulação diferente, com resultados diferentes (Isto para os mesmos métodos), sem contar que o armazenamento também pode diferir.
...
Os métodos mudam exatamente por conta das diferenças apresentadas entre as estruturas de dados, assim, o `.loc` e o `.iloc` permitem a busca por linhas e colunas nos `DataFrames` e somente das linhas de uma `Series`. 
...
As agregações apresentam diversas derivações de definição, a utiliza aqui será a seguinte: "Operações aplicadas sobre os dados que resultam em um conjunto de valores". 

No pandas, o retordo de uma agregação pode mudar de acordo com a estrutura de dados onde a operação é aplicada, no caso das `Series` é retornado apenas um valor. Para os `DataFrames`, um valor para cada linha/coluna é retornado.
...
Aqui vamos focar no agrupamento dos `DataFrames`.
...
Dentro do `DataFrameGroupBy` existe o atributo `groups` que possibilita verificar a lista com cada grupo gerado e seus membros.
...
Além disto, com um objeto do tipo `DataFrameGroupBy` é possível aplicarmos os conceitos de agregação dentro dos grupos que foram gerados através do `groupby`.
