...
Viu ? É tudo muito simples e direto, o que te permite focar em sua análise e deixar de lado problemas com sintaxe `r emo::ji("statue_of_liberty")`. Para fechar esta seção de análise de dados com Python e Pandas, vejamos uma análise completa, aplicável ao mundo real utilizando todos os conceitos que vimos até aqui.

## Exemplo - Análise de dados com Python

Para a realização desta análise de exemplo foi utilizado o conjunto de dados [Meteorite Landings](https://www.kaggle.com/nasa/meteorite-landings), com registros de mais de 45 mil meteoros que atingiram a Terra. Os dados foram retirados da plataforma [kaggle.com](https://www.kaggle.com/).

Para iniciar a análise, da mesma forma que foi feito nos exemplos, é necessário import o pandas
...
Beleza, um `DataFrame`, isto indica que vamos conseguir aplicar os métodos que vimos anteriormente sem problemas. Agora vamos verificar as dimensões que a tabela representa no `DataFrame` possui. Para tal, o atributo `shape` será utilizado.
...
Interessante! Os valores `(45716, 10)`, indicam respectivamente que, o conjunto de dados possuí `45716` linhas e `10` colunas. Por falar nas colunas, vamos verificar quais são cada uma das colunas disponíveis neste conjunto de dados.
...
> A descrição de cada coluna está disponível na [página dos dados](https://www.kaggle.com/nasa/meteorite-landings), não deixe de conferir para um entendimento completo sobre os métodos de análise e os dados
...
- Relict: Meteorito degrado pelo clima da Terra.

Façamos a separação do conjunto de dados, para verificar qual tipo possui mais dados.
...
Olha que bacana! Já foi possível perceber que, os meteoritos `Valid` são apresentados em escalar muito maiores. Agora, vamos entender a quantidade de massa média que cada um desses grupos apresenta.
...
Depois de agrupar, vamos olhar o resultado
...
É isto, este foi o exemplo para mostrar que com os poucos métodos que vimos neste curso de introdução, é possível tirar várias informações do conjunto de dados, desde que, seja possível entender a estrutura e semântica dos dados.
