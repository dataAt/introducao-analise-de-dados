...
Agora que já sabemos como declarar uma variável, um vetor e um *Data frame*, nessa subseção vamos dar algumas dicas e falar sobre alguns métodos estatísticos.
...
Para verificar o desvio padrão do conjunto, podemos utilizar a função `sd()` do pacote `stats` (já incluso no R). Podemos verificar a média com `mean()` que é uma função do pacote `base` e a mediana com `median()` do pacote `stats`.
...
Os `IF's` da vida, se não fosse por isso, aconteceria tal coisa. No **R**, a estrutura de decisão é bem parecida com a do *Java*, ainda com o conjunto do *star wars*:
...
Para encerrarmos esse capítulo de R, vamos mostrar uma análise aplicável ao mundo real, com uso de dois conjuntos de dados, obtidos através da plataforma [kaggle](https://www.kaggle.com/). Os conjuntos de dados utilizados correspondem a série temporal de [mudança climática da supércie terrestre](https://www.kaggle.com/berkeleyearth/climate-change-earth-surface-temperature-data) e os [continentes de cada país](https://www.kaggle.com/statchaitya/country-to-continent). O objetivo desta análise é verificar qual o continente que registrou a maior temperatura anual.
...
```{r, message=FALSE}
  ...
  # Filtro a partir dos anos 2000 e tirando a média anual (talvez não seja a melhor aboragem)
...
```{r}
  # Criação de um grupo de continentes e agregando pela meior temperatura
  final_dataset %>%
  dplyr::group_by(continent) %>% 
  summarise(maior_temp = max(year_mean, na.rm = TRUE))
```

Podemos gerar outras peguntas, por exemplo, qual o ano com a maior média de temperatura registrada?

```{r}
  # Criação de um grupo de anos e agregação pelo valor da temperatura, por fim ordenação pelos valores de tempearatura
  final_dataset %>%
  dplyr::group_by(dt) %>% 
