...
[CRAN](https://cran.r-project.org/) - *C*omprehensive **R** **A**rchive **N**etwork. Abaixo é apresentada a sintaxe para instalar pacotes em R:
...
Então, segundo a ideia de hierarquia de tipos do **R**, o vetor da célula acima será convertido para o tipo `character`e continuará sendo atômico `r emo::ji("drum")`.
...
A seguir é apresentado um exemplo com uma lista não atômica, ou seja, uma lista heterogênea composta de vários tipos. Mas, será ela uma lista mesmo ? `r emo::ji("man_vampire_dark_skin_tone")`
...
Um **Data Frame** pode ser entendido como uma matriz (Figura \@ref(fig:fig233)) composta por linhas e colunas, cujas colunas representam as variáveis (atributos) e as linhas representam observações. A Figura \@ref(fig:fig322) apresenta um **Data frame** onde é possível observar as definições mencionadas anteriormente. A partir da imagem podemos inferir que o aluno **José** obteve **7** na disciplina de **Álgebra Linear**. 
...
Vamos utilizar o pacote `dplyr` do *kit* de ferramentas [**Tidyverse**](https://www.tidyverse.org/) para manipular os valores de um `data.frame`. Para isso, criamos o seguinte `data.frame`:
